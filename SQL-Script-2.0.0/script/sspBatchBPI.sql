...
declare @BPI1num bigint
declare @BPI2num bigint
declare @BPI3num bigint
...
(select @BPI1 = ItemListID, @BPI1num = TotalCount from (select ItemListID, row_number() over(order by t1.TotalCount desc) as rownum, TotalCount from (select TOP 3 ItemListID, count(*) AS TotalCount from MemberItemPurchases where PurchaseDT between @Last30DT and @CurrentDT group by ItemListID order by TotalCount desc) t1) t2 where t2.rownum like '1')
(select @BPI2 = ItemListID, @BPI2num = TotalCount from (select ItemListID, row_number() over(order by t1.TotalCount desc) as rownum, TotalCount from (select TOP 3 ItemListID, count(*) AS TotalCount from MemberItemPurchases where PurchaseDT between @Last30DT and @CurrentDT group by ItemListID order by TotalCount desc) t1) t2 where t2.rownum like '2')
(select @BPI3 = ItemListID, @BPI3num = TotalCount from (select ItemListID, row_number() over(order by t1.TotalCount desc) as rownum, TotalCount from (select TOP 3 ItemListID, count(*) AS TotalCount from MemberItemPurchases where PurchaseDT between @Last30DT and @CurrentDT group by ItemListID order by TotalCount desc) t1) t2 where t2.rownum like '3')

if @BPI1 is not null
	begin
		insert into StatsData(CategoryName, CountNum, Fields, Groups) values('BPItem1', @BPI1num, CONVERT(nvarchar(8), GETUTCDATE(), 112), @BPI1)
	end
if @BPI2 is not null
	begin
		insert into StatsData(CategoryName, CountNum, Fields, Groups) values('BPItem2', @BPI2num, CONVERT(nvarchar(8), GETUTCDATE(), 112), @BPI2)
	end
if @BPI3 is not null
	begin
		insert into StatsData(CategoryName, CountNum, Fields, Groups) values('BPItem3', @BPI3num, CONVERT(nvarchar(8), GETUTCDATE(), 112), @BPI3)
	end
GO
